...
- `scripts/` - `Poser.lua` (runtime with `curve = "spline"`, `lag`,
  ...
  leap strike and a four-hit M1 string), `EditStrip.lua` (Edit-mode pose strips and the foot
  ...
  (`Poser.check`, `Poser.dump` and timed `Rig` calls on the Luau command line
  tool, no Studio),
