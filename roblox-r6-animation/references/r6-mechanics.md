...
The spline and the eases run on the channels, not on the CFrame, so an equal pose keyed at another branch takes another path. An overhand throw keys the cocked arm at lift -130 and the release at -230 so the arm goes up over the head; keying the release as its equal, +130, swings it down and under. The clip may end at -305 for a rest of 55: Poser blends CFrames, so the next clip is not affected. Beyond lift 90 or below -90 the side channel turns the other way (x = cos(lift) sin(side)): for a right arm up and behind the head, outward is negative side. `scripts/ExampleThrow.lua`.
