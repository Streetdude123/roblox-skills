...
4. Get a reference and study it frame by frame before planning ([reference-study.md](references/reference-study.md)): a video the user sends, a clip in the place, or reference media you find. For a cutscene or any cinematic request also read [cinematic-shots.md](references/cinematic-shots.md).
...
Count at 60 fps and write the beat table: anticipation (in stages for a big action), strike or action, contact event, follow-through, recovery, exit pose. Record which feet are planted and which part leads each beat. Take the frame counts from the reference study; use the timing ranges in principles.md only where there is no reference, and the gameplay contract for event times. Write the character's weight (stance depth, skid, recovery time, time in the air) before the first key. A chained move ends on the next move's first pose.
...
2. Play the clip on the character in Play mode and record a take from the player's camera (pipeline.md, "Video of a clip"). Sheet every unique frame with `scripts/video/ref_sheets.py` and put the sheet next to the reference's sheet at the same beats ([reference-study.md](references/reference-study.md)). Write the differences: key poses, timing per beat, spacing, arcs, holds, overlap, weight, smears, staging. Where recording is not possible, capture at the contact and the recovery and say so.
