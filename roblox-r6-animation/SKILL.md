...
- Faults: `python3 scripts/faults.py <decode>` on the `Poser.dump` text (from Studio or `poser_offline.py`) must show nothing unexplained. It flags twinning arms, an arm hanging dead under a lean, a pose near the default stand mid-clip, a turn of more than 90 degrees in one frame (it needs a smear), a part through the floor, a planted leg lying flat or twisted off the torso, a hip gap or a foot slide, and an arm, the head or the torso that is fastest away from the strike (`--strike <seconds>` names it; `--float` skips the feet on a stand). A flag you keep on purpose (a kneel's flat leg, a whip that gets a smear) goes in the report with its reason.
