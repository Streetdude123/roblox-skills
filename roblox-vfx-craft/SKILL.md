...
- **Use Toolbox VFX kits OFTEN** ("you have to be using VFX kits OFTEN from the toolbox, this is to maximize your vfx
  quality", 2026-09-26). For every new effect, search the Creator Store (`search_asset`, Model, free) for packs that fit
  (water swirls, magic circles, explosions, slashes, auras) before you hand-roll a piece. Insert into `ServerStorage` (scripts
  there never run), list every script and read it, delete scripts and sounds, render the pieces at real size, keep only the
  pieces you use in the builder's source folder, delete the rest of the pack (his machine has 6 GB). Hand-roll only the glue.
...
- 2026-09-26 (Grimoire Duelists): "widen the parrying window and make it easier to parry because it's so hard to react, make sure the diffrientation between blocking and parrying is obvious please", "make sure spamming F puts you in parry mode, parry mode lasts 2 seconds, only the start of parry mode is parryable" (window 0.45 s), "add a way to be able to see attacks coming easier and attacks that are unparryable should be signalled using vfx" (two signals), "you decide just make it look cool and difference clear". Built `AstaTell`: a four-point star glint (10598374841) with a glow (1075864321) on the weapon at the start of every attack, locked to the blade or wand tip: white for normal, amber with a ring for block breakers, red with a ring, dark smoke and a double flash for unparryable. First sizes (star 3.6) read as specks from 26 studs; 6 / 7.5 / 9 read. Parry and block now differ by value, not only size: parry = a white-ice star flash, spike burst, ring and light in both kits; block = the kit colour only (dim orange sparks and dust for the sword, a blue splash for water, the white flash removed). Also "moves like bull thrust into walls or anchored objects flings you": a plane LinearVelocity at 1e6 pushing into a wall or a body sends the contact energy up; cast rays ahead each frame and slide the move along the wall, and let bodies pass through each other (his pick).
- 2026-09-26 (Water Mage ultimate): "Let's do dragon form with leviathan ... we have to make this look SUPER COOL though okay? like animation and VFX at a high professional level", picks: timed form (20 s, 60 s cooldown) on Z (and Asta's ultimate moved to Z), full dragon form, hover and glide, lance combo, R and T in form. The transform cutscene (Leviathan spiral, roar, dive, cocoon, burst) was accepted: "the vfx animation for turning into the dragon is good". The form itself was not: "Im not really liking how it looks, make the VFX SIGNIFICNATLY COOLER, ITS WATER this looks horrid", "it's rhe form itself that looks horrible", "Make sure it's not geometric okay, VFX NEEDS TO LOOK AMAZING IN THHIS FORM USING WATER VFX, the design of the form can differ a bit from the canon anime", "Make sure it's more natural for the character, and more flashier", "Make sure these horns correctly go in the head" (a screenshot of the horns floating beside the head), "make those little chains that wrap around the character more cooler and natural now, i's like if more of the body was covered please", then "No stop it needs to cover in a way where the design of the VFX is cooler not like rhat" and "And not the whole body, you're covering the whole body". Picks: wraps on the forearms and hands, shoulders, chest and collar, as trailing ribbons. What changed and why is in water.md section 9.
- 2026-09-26 (Dragon Form move variants): "remember you can use free model vfx kits, please", then "in your vfx skill, note that you have to be using VFX kits OFTEN from the toolbox, this is to maximize your vfx quality". The first use: Ludic Sanctuary is built from "Hybrid's Water VFX Pack" (12280564151: a layered Neon water swirl, mesh 6063631009, and a swirling water ball, mesh 5747850601; its only script was `script.Parent:remove()` on a thumbnail camera) and the "Rune" model (12086396880: a Neon line-art rune circle, no scripts, one sound removed). A flat decal circle from the store looked low resolution and dim next to them; the model pieces read at once. Also from this pass: "make the three heada branch off so it's not clutteres" (one water trunk from the raised lance splits into three necks; three heads spiralling round the caster read as one tornado), "keep the water blades on the normal form, it's only different in ult form, same with all the other stuff", "don't record by the way".
- 2026-09-26 (Dragon Form lance combo): "Rework the animation by the way for the m1's and the skewer need to spin the enemy around quick and launch them away" and "nice animations but make sure the enemy is fling up with less power so that they don't fly away too far". The skewer spin carries the kit's WaterWhirl around the body; its near-black mesh read as black ink and it hid the enemy, so kit pieces now get the black parts recoloured to deep water blue and the whirl runs small and see-through (water.md, section 10). A thrown enemy must leave with the throw knockback only: zero the spin speed at the release (it flew 244 studs, now 19.5).
